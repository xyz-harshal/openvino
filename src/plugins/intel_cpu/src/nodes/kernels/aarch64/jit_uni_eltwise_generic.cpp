    ...
    if (min_src_size != jep.dst_size) {
        ...
        L(unroll_loop_label);
        {
            ...
            for (size_t j = tail_start; j < min_src_size; j++) {
                for (size_t i = 0; i < jep.inputs_number; i++) {
                    if (jep.src_size[i] != 1) {
                        load_scalar(get_scl_reg(i),
                                    get_src_reg(i),
                                    jep.src_prc[i],
                                    exec_prc,
                                    j * jep.src_prc[i].size());
                    }
                }
...
void jit_uni_eltwise_generic<isa>::load_vector(const TReg& data,
    ...
    if (dst_prc != src_prc) {
        ...
        case ov::element::f32:
            ...
            default:
                OPENVINO_THROW("src_prc " + src_prc.to_string() + " is not supported, dst_prc is " +
                               dst_prc.to_string());
            }
            break;
        default:
...
void jit_uni_eltwise_generic<isa>::store_vector(const XReg& ptr,
    ...
    if (src_prc != dst_prc) {
        ...
        case ov::element::f32: {
            ...
            default: {
                OPENVINO_THROW("dst_prc " + dst_prc.to_string() + " is not supported, src_prc is " +
                               src_prc.to_string());
            }
            }
            break;
